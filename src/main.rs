...
//! each in its own PTY, monitored and steered from a single screen.
//!
//! Built on [`ratatui-ppalla`](https://crates.io/crates/ratatui-ppalla) (the
//! Preparable-pattern rendering primitives). The interactive loop, PTY
//! management, terminal emulation and AgentBus are all wired together in the
//! [`app::App`] — see [`app`] for the threading model.
...
enum Command {
    ...
    ///
    /// Splits the spec into tasks (one per non-empty line) and prints the
    /// dispatch plan across the detected (or default) agents. Live dispatch
    /// wiring is a follow-up; this exposes the coordination model.
    Orchestrate {
    ...
    /// URL and a one-time pairing token. (Live snapshot publishing from a
    /// running App is a follow-up; the server itself is functional.)
    Mobile {
...
fn try_main(cli: Cli) -> Result<()> {
    match cli.command {
        Command::Run {
            ...
            if let Some(host) = &remote {
                ...
                for spec in &mut specs {
                    spec.command = target.clone().with_command(spec.command.clone()).command_vec();
                }
            }
            ...
            if let Some(port) = mobile {
                ...
                std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        let _ = runtime.block_on(mobile::serve(addr_for_server, token_for_server, snap_rx));
                    })?;
                eprintln!(
        ...
        Command::Mobile { port } => {
            let token = mobile::random_token();
            let addr: SocketAddr = format!("0.0.0.0:{port}").parse()?;
            // No live App snapshot feed yet; serve with an empty channel so the
            // server is up and accepting connections (clients hold the socket).
...
fn split_agents(args: Vec<String>) -> Vec<Vec<String>> {
...
mod tests {
    ...
    fn split_agents_drops_empty_segments() {
        // Leading, trailing, and doubled `::` all yield empty segments that
        // must be dropped (they would otherwise spawn an agent with no binary).
        let got = split_agents(vec![
            "::".into(),
            "echo".into(),
            "::".into(),
    ...
    /// End-to-end check of the `try_main` spec-building path: splitting + the
    /// empty guard. Keeps the error message in sync with the `::` wording.
    #[test]
    fn smoke_parse_three_agents_via_separator() {
        ...
        // `echo`/`true`/`false` are not known agent binaries → all Generic.
        for s in &specs {
